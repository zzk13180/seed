...
const makeAvailableArr = (disabledList: boolean[]): number[] => {
  ...
  const getNumber = (predicate: number | true): predicate is number => predicate !== true
...
export const getTimeLists = (
  ...
  const getSecondsList = (hour: number, minute: number, role: string, compare?: Dayjs) => {
...
export const buildAvailableTimeSlotGetter = (
  ...
  const getAvailableSeconds: GetDisabledSecondsState = (hour, minute, role, compare?) => {
