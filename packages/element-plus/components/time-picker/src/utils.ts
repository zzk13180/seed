...
export const buildTimeList = (value: number, bound: number): TimeList => {
  return [value > 0 ? value - 1 : undefined, value, value < bound ? value + 1 : undefined]
}
...
export const valueEquals = function (a: Array<Date> | unknown, b: Array<Date> | unknown) {
...
export const dayOrDaysToDate = (dayOrDays: DayOrDays): DateOrDates => {
  return isArray(dayOrDays) ? (dayOrDays.map(d => d.toDate()) as [Date, Date]) : dayOrDays.toDate()
}
