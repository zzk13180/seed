...
<script lang="ts" setup>
  ...
  const emitChange = (val: TimePickerDefaultProps['modelValue'] | null, isClear?: boolean) => {
  ...
  const parsedValue = computed(() => {
    ...
    if (valueIsEmpty.value) {
      ...
      dayOrDays = props.modelValue.map(d => parseDate(d, props.valueFormat, lang.value)) as [
        Dayjs,
        Dayjs,
      ]
    } else {
  ...
  const onTouchStartInput = (event: TouchEvent) => {
    ...
    if ((event.touches[0].target as HTMLElement)?.tagName !== 'INPUT' || isFocused.value) {
      pickerVisible.value = true
    }
  }
  ...
  const stophandle = onClickOutside(inputRef as Ref<ComponentPublicInstance>, (e: PointerEvent) => {
  ...
  const handleStartChange = () => {
    ...
    if (value && value.isValid()) {
      userInput.value = [formatDayjsToString(value) as string, displayValue.value?.[1] || null]
  ...
  const handleEndChange = () => {
    ...
    const parsedVal = unref(parsedValue) as [Dayjs, Dayjs]
    if (value && value.isValid()) {
      userInput.value = [unref(displayValue)?.[0] || null, formatDayjsToString(value) as string]
  ...
  // @ts-ignore
  const onPanelChange = (value: [Dayjs, Dayjs], mode: 'month' | 'year', view: unknown) => {
