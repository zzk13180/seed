...
function useRender<T>(props: Partial<TableBodyProps<T>>) {
  ...
  const { getRowStyle, getRowClass, getCellStyle, getCellClass, getSpan, getColspanRealWidth } =
    useStyles(props)
  ...
  const rowRender = (row: T, $index: number, treeRowData?: TreeNode, expanded = false) => {
