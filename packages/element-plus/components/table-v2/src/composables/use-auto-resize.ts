...
const useAutoResize = (props: AutoResizerProps) => {
  ...
  onMounted(() => {
    resizerStopper = useResizeObserver(sizer, ([entry]) => {
      ...
      const left = Number.parseInt(paddingLeft, 10) || 0
      const right = Number.parseInt(paddingRight, 10) || 0
      const top = Number.parseInt(paddingTop, 10) || 0
      const bottom = Number.parseInt(paddingBottom, 10) || 0
