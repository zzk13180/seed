import { isNumber, throwError } from '@seed/element-plus-utils'
...
const FixedSizeGrid = createGrid({
  ...
  getColumnOffset: (
    ...
    const minOffset = Math.max(
      0,
      columnIndex * (columnWidth as number) - width + scrollBarWidth + (columnWidth as number),
    )
  ...
  getRowOffset: (
    ...
    const minOffset = Math.max(
      0,
      rowIndex * (rowHeight as number) - height + scrollBarWidth + (rowHeight as number),
    )
  ...
  getRowStartIndexForOffset: ({ rowHeight, totalRow }, scrollTop: number): number =>
