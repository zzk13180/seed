  ...
  return defineComponent({
    ...
    setup(props, { emit, expose, slots }) {
      ...
      const emitEvents = () => {
        ...
        if (totalColumn! > 0 && totalRow! > 0) {
          const [columnCacheStart, columnCacheEnd, columnVisibleStart, columnVisibleEnd] =
        ...
        const { scrollLeft, scrollTop, updateRequested, xAxisScrollDir, yAxisScrollDir } =
          unref(states)
        emit(SCROLL_EVT, {
          xAxisScrollDir,
          scrollLeft,
      ...
      const renderScrollbars = () => {
        const { scrollbarAlwaysOn, scrollbarStartGap, scrollbarEndGap, totalColumn, totalRow } =
          props
