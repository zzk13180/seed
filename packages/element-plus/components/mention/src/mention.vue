...
<script lang="ts" setup>
  ...
  const handleInputKeyDown = (event: KeyboardEvent | Event) => {
    if (!('code' in event) || elInputRef.value?.isComposing) {
      return
    }
    ...
    switch (event.code) {
      ...
      case EVENT_CODE.down:
        ...
        dropdownRef.value?.navigateOptions(event.code === EVENT_CODE.up ? 'prev' : 'next')
        break
  ...
  const getInputEl = () =>
    props.type === 'textarea' ? elInputRef.value?.textarea : elInputRef.value?.input
