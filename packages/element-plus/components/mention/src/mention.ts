...
export const mentionProps = buildProps({
   ...
   */
  checkIsWhole: {
    type: definePropType<(pattern: string, prefix: string) => boolean>(Function),
  },
  /**
...
export const mentionEmits = {
  ...
  search: (pattern: string, prefix: string) => isString(pattern) && isString(prefix),
  select: (option: MentionOption, prefix: string) => isString(option.value) && isString(prefix),
