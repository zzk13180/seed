...
import { WEEK_DAYS } from '@seed/element-plus-constants'
...
export const useDateTable = (props: DateTableProps, emit: SetupContext<DateTableEmits>['emit']) => {
  ...
  const rows = computed(() => {
    ...
    if (isInRange.value) {
      ...
      const currentMonthRange: CalendarDateCell[] = rangeArr(end.date() - start.date() + 1).map(
        index => ({
      ...
      const currentMonthDays: CalendarDateCell[] = getMonthDays(props.date).map(day => ({
        text: day,
        type: 'current',
      }))
