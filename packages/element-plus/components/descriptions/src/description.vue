...
<script lang="ts" setup>
  ...
  import { flattedChildren } from '@seed/element-plus-utils'
  ...
  const filledNode = (node: DescriptionItemVNode, span: number, count: number, isLast = false) => {
    if (!node.props) {
      node.props = {}
    }
    if (span > count) {
  ...
  const getRows = () => {
    ...
    const children = flattedChildren(slots.default()).filter(
      (node): node is DescriptionItemVNode => (node as any)?.type?.name === 'ElDescriptionsItem',
    )
