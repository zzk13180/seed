...
<script lang="ts" setup>
  ...
  const isValidValue = (date: [Dayjs, Dayjs]) => {
    return (
      isValidRange(date) &&
      (disabledDate ? !disabledDate(date[0].toDate()) && !disabledDate(date[1].toDate()) : true)
    )
  }
  ...
  const formatEmit = (emitDayjs: Dayjs | null, index?: number) => {
    ...
    if (defaultTime) {
      const defaultTimeD = dayjs(defaultTime[index as number] || defaultTime).locale(lang.value)
