...
| **类型来源** | Zod schemas (`@seed/kit/schemas`) | `z.infer<>` 推导类型，前后端共享 |
...
- **Types**: 类型从 `@seed/kit/schemas` 的 Zod schema 通过 `z.infer<>` 推导，同时 re-export schema 供 View 层表单验证。
...
- **类型来源**：`*.types.ts` 中从 `@seed/kit/schemas` 导入 Zod schema，用 `z.infer<>` 推导 DTO 类型，前后端共享同一份验证规则。
...
```typescript
import { z } from 'zod'
import { panelDataSchema } from '@seed/kit/schemas/panel'
